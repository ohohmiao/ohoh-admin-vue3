<template>
	...
	>
		...
		>
			<el-form-item label="流程按钮" prop="btnId">
				<el-select v-model="formProps.rowData.btnId">
					<el-option v-for="item in btnSelectDatas" :key="item.value" :value="item.value" :label="item.label"></el-option>
				</el-select>
			</el-form-item>
...
import { WorkflowBtn, getWorkflowBtnSelectApi } from "@/api/modules/workflow/btn";
...
interface FormProps {
	...
	title: string;
	isView: boolean;
	rowData: Partial<WorkflowBtn.BindForm>;
...
const rules = reactive({
	btnId: [{ required: true, message: "请选择流程按钮" }],
	bindNodeNames: [{ type: "array", required: true, message: "请选择绑定环节" }]
});

// 流程按钮下拉框数据
const btnSelectDatas = ref<{ label: string; value: string }[]>([]);
onMounted(async () => {
	const { data } = await getWorkflowBtnSelectApi();
	btnSelectDatas.value = data;
});
